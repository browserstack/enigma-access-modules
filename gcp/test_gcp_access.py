...
def test_grant_gcp_access(mocker, test_name, user_email, group_id, domain_id, expected_return_value, google_client):
...
def test_revoke_gcp_access(mocker, test_name, user_email, group_id, domain_id, expected_return_value, google_client):
  mocker.patch("gcp.helpers.get_gcp_client", return_value=google_client)
...
def test_GCPAccess(mocker):
  ...
  assert combine_label_desc == "GroupAccess for groups: " + ", ".join(form_label[0]["groups"])
