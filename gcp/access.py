...
class GCPAccess(BaseEmailAccess):
        ...
        try:
            ...
            for access_label_data in access_labels_data:
                valid_access_label = {
                    ...
                    "groups": access_label_data["groups"],
                }
        ...
        for label in labels:
            for group in label["groups"]:
                result, exception = helpers.grant_gcp_access(
                    group, label["domain"], user.email
                )
                if result is False:
                    logger.error(
                        f"Something went wrong while adding the {user.email} to group {group}: {str(exception)}"
                    )
                    return False
    ...
    def revoke(self, user, label, request):
        for group in label["groups"]:
            ...
            if not result:
                logger.error(
                    f"Error while removing the user from the group {group}: {str(exception)}"
                )
                return False
