...
@given('user is not invited earlier for workspace',
       target_fixture="user_not_invited_output")
def user_not_invited_earlier(mocker):
    """user is not invited earlier for workspace."""
    ...
    client_mock.admin_users_invite = mocker.MagicMock(
...
def user_identity(mocker):
    """User Identity Mock."""
    user_identity_mock = mocker.MagicMock()
    user_identity_mock.user.email = user_email()
    return user_identity_mock
