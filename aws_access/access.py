...
class AWSAccess(BaseEmailAccess):
    ...
    def approve(
        ...
        for label in labels:
            for group in label["groups"]:
                granted_access, exception = helpers.grant_aws_access(
                    user, label["account"], group
                )

                if not granted_access:
                    logger.error(
                        f"Something when wrong while adding {user.email} to group {group}: {str(exception)}"
                    )
                    return False
        ...
        if auto_approve_rules:
            ...
            email_subject = (
                ...
                % (request.request_id, label_desc, user.email, approver)
            )
    ...
    def revoke(self, user, label, request):
        for group in label["groups"]:
            ...
            if not is_revoked:
                logger.error(
                    f"Something went wrong while removing {user.email} from {group}: {str(exception)}"
                )
                return False
    ...
    def validate_request(self, access_labels_data, request_user, is_group=False):
        ...
        for access_label_data in access_labels_data:
            ...
            if access_label_data["action"] == constants.GROUP_ACCESS:
                ...
                if not helpers.aws_group_exists(access_label_data.get("groups")):
            ...
            valid_access_label = {
                ...
                "groups": access_label_data.get("groups"),
            }
