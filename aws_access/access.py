...
class AWSAccess(BaseEmailAccess):
    ...
    def approve(
        ...
        if auto_approve_rules:
            email_subject = (
                "Access Granted: %s for access to %s for user %s.<br>Request has been"
            ...
            email_subject = (
                "Access Granted: %s for access to %s for user %s.<br>Request has been"
                " approved by %s." % (request_id, label_desc, user.email, approver)
            )
    ...
    def revoke(self, user, label):
        logger.info(
            f'[{datetime.now().strftime("%Y-%m-%d")}] [{self.tag()}] Revoke'
            f" Started({user.email}) : {label}"
        )
        is_revoked, exception = helpers.revoke_aws_access(user.user, label)
        logger.info(
            f'[{datetime.now().strftime("%Y-%m-%d")}] [{self.tag()}] Revoke'
            f" Result({user.email}) : {is_revoked}"
        )
    ...
    def fetch_access_approve_email(self, request, data):
        ...
        return str(
            render(
                request, "aws_access/accessApproveEmail.html", context_details
