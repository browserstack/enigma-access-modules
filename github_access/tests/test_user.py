...
def user_does_not_exist(requests_mock, context):
    ...
    context['expected_message'] = "Username test-username not present on github!"
...
def user_invited(requests_mock, context):
    API_URL = f'https://test-base-url.com/orgs/test-org/invitations'
    expected_headers = {'Authorization': 'token test-token'}
    ...
    assert return_value == True
    context['expected_message'] = "Already invited test-username to join github org. Please accept the invite first."
...
def cannot_add_user_to_org(requests_mock, context):
    ...
    API_URL = f'https://test-base-url.com/orgs/test-org/memberships/test-username'
    expected_headers = {'Authorization': 'token test-token'}
    ...
    assert return_value == False
    context['expected_message'] = "Failed adding user test-username to github org"
...
def add_user_to_org(requests_mock, context):
    ...
    return_value = helpers.put_user(username=user_name())
    assert return_value == True
    context['expected_message'] = "Invited test-username to join github org. Access can be granted post inivation acceptance."
