...
def get_org_repo_list_success(requests_mock):
    API_URL = f'https://test-base-url.com/orgs/test-org/repos'
    expected_headers = {
...
def validate_output(context_output):
    return_value = context_output
    assert return_value == [{'action': 'repository_access', 'access_level': 'merge', 'repository': 'org/repo1'}, {'action': 'repository_access', 'access_level': 'merge', 'repository': 'org/repo2'}]
