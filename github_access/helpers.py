...
def _get_branch_protection_enabled(repo, branch):
    ...
    if 'restrictions' in protected_branch_data and \
            'required_pull_request_reviews' in protected_branch_data:
        return True
    return False
...
def grant_access(repo, access_level, username):
    ...
    try:
        ...
        if not get_repo(repo):
            logger.debug(
                'Skipping git access for %s for user %s because repo does not exist anymore' % (
                    repo, username))
            return True
        if access_level == 'merge':
            ...
            for branch in default_branch:
                response = ""
                GET_USERS_ACCESS_URL = '%s/repos/%s/branches/%s/protection/restrictions/users' % (
                    GITHUB_BASE_URL,
                    repo,
                    branch)
...
def _is_protection_enabled(repo, branch, protected_branch_data, username, response):
    if _get_branch_protection_enabled(repo, branch):
        ...
        headers = {
            ...
            'Accept': 'application/vnd.github.v3+json',
            'Content-Type': 'application/json'}
        POST_USERS_ACCESS_URL = '%s/repos/%s/branches/%s/protection/restrictions/users' % (
            GITHUB_BASE_URL,
            repo,
            branch)
        res = requests.post(POST_USERS_ACCESS_URL, headers=headers, data=json.dumps(data))
        response = res.json()
    elif "message" in protected_branch_data and \
            protected_branch_data["message"] == "Push restrictions not enabled":
        response += f"404 - Push Restriction is not enabled for {repo} Repo. \
            Please contact Github Admin to enable branch protection for this repo"
